...
function buildCave() {
  ...
  const L1 = [
    ...
    [-16.2, -14, -14, -11], // 上層への坂の頭上空間 (西: 坂の最上段ぶんだけ開ける)
    [14, 16.2, 11, 14],     // 上層への坂の頭上空間 (東)
  ];
  ...
  boxes.push(...stairs(34.5, 0, 0, 'e', 3.4, 16, SH, SD, RS));    // 東の洞窟 → 東回廊 (最奥まで埋めて穴を作らない)
  boxes.push(...stairs(4, 0, 34.5, 's', 3.4, 16, SH, SD, RS));    // 南の洞窟 → 南回廊
  boxes.push(...stairs(-34.5, 0, 0, 'w', 3.4, 16, SH, SD, RS));   // 西の洞窟 → 西回廊 (同上)
