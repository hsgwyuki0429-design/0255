...
export class Humanoid {
  constructor(colorIdx = 0, name = '') {
    ...
    const bean = capsule(0.35, 0.76, c, -0.21);
    ...
    for (const s of [-1, 1]) {
      ...
      eye.position.set(s * 0.11, 0.06, 0.3);
      this.spine.add(eye);
    }

    const stem = capsule(0.02, 0.13, sproutStem, 0.0); this.neck.add(stem);
    this.neck.add(ball(0.055, sproutBall, 0.12, 1, 1.1, 1));
    for (const s of [-1, 1]) {
    ...
    for (const s of [-1, 1]) {
      ...
      shoulder.position.set(s * 0.29, 0.02, 0.02); this.chestG.add(shoulder);
      shoulder.add(capsule(0.05, 0.08, c, -0.09));
      const elbow = new THREE.Group(); elbow.position.y = -0.18; shoulder.add(elbow);
      elbow.add(ball(0.055, c, -0.05, 1, 0.9, 1));
  ...
  setName(name) {
    ...
    this.tag.scale.set(2.7, 0.675, 1);
    this.tag.position.y = 4.1;
  ...
  update(st) {
    ...
    if (!st.onGround) {
      ...
      this.pelvis.rotation.x = 0.06 + runK * 0.28;
      ...
      this.neck.rotation.z = swing * 0.12;
    } else {
