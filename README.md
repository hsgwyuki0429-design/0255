...
- 鬼の銃は **約1mしか飛ばない超短射程のペイント弾**。当たると相手の体にインクの痕が残る。
...
- CPUレートが上がると、CPUは足が速くなるのではなく**思考が賢くなる**:
  反応速度・経路探索・偏差撃ち・逃げ道の吟味・仲間の救出・牢屋の見張りが段階的に強化される。
...
- **地下洞窟** ─ 岩盤に**坑道が縦横に掘られたネットワーク構造**。中央広場・4隅のチェンバー・外周坑道・ジグザグ連絡坑がすべてループし、木の支保工とクリスタルが道を照らす。牢屋は骨の檻。
- **ショッピングモール** ─ 吹き抜けアトリウムの2フロア構造。エスカレーター階段2基+バック階段、店舗はすべて2ドア、2Fの手すりは飛び越えて1Fへ落下ショートカット可。牢屋は警備室。
- **学校** ─ **U字型4階建て校舎**。北の本棟から東西のウイングが伸び、囲まれた**中庭**には池と桜。両端の**折り返し階段室**で1F〜4F〜屋上まで繋がり、屋上フェンスを飛び越えて中庭へダイブもできる。**体育館**(ステージ・跳び箱・器具倉庫=牢屋)と校庭(ジャングルジム・トランポリン)付き。
